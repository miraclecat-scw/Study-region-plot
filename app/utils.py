...
def coords_npy_to_lonlat(coords_npy_path):
    """
    读取保存为 npy 的 coords_list，并返回 lon, lat 数组。

    支持格式：
    1. np.array([[lon, lat], [lon, lat], ...])
    2. np.array([(lon, lat), (lon, lat), ...])
    3. object array 的 list/tuple 格式
    """

    coords = np.load(coords_npy_path, allow_pickle=True)

    coords = np.asarray(coords)

    # 如果是 object array，比如保存的是 list of tuple
    if coords.dtype == object:
        coords = np.array(coords.tolist(), dtype=float)

    # 期望最终 shape 为 [N, 2]
    if coords.ndim != 2 or coords.shape[1] != 2:
        raise ValueError(
            f"coords should have shape [N, 2], but got {coords.shape}"
        )
